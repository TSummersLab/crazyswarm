...
class state():
    def __init__(self, pose, twist, acc):
        self.pose = pose
        self.twist = twist
        self.acc = acc
...
def main(args):
    ...
    k_dist = 0.3 #0.3 # multiplier for distance control gain obtained from calculating the gains matrix (A) in matlab
    kp_ctrl = 0.07 #0.3 # Proportional control gain (used when discritizing velocity control)
    d_sat = 0.05 #0.07 # saturation distance (max distance that can be traveled in the given time~sleep time)
    iteration_length = 1.# length of an iteration in seconds (for discritizing the trajectory)
    sleep_time = 0.05
    k_vel = 0.07 #0.2 # Proportional velocity control gain
    v_sat = 0.03 #0.2 # maximum velocity in m/s
    ...
    form_ctrl_shape = '8_agent_struct_1'
    ...
    while not rospy.is_shutdown():
        ...
        ctrl = np.zeros([num_agents, 3]) # position control
        for agent in range(num_agents):
        ...
        vel_ctrl =  np.zeros([num_agents, 3]) # velocity control
        for agent in range(num_agents):
            if not stopFlag_old[agent]:
                vel_ctrl[agent] = k_vel * dq_in_world[agent]

            v_norm = np.linalg.norm(vel_ctrl[agent])
            if v_norm > v_sat:
                vel_ctrl[agent] = vel_ctrl[agent]/v_norm * v_sat # saturate if needed
                print('Vel Ctrl SAT: ', vel_ctrl[agent])
        ...
        # un-permute the vel control (undo effect of E) to send the correct control signal to the correct robot
        # Stack the control
        vel_ctrl_original_stack = np.hstack(vel_ctrl) # array with positions stacked (x1,y1,z1, x2,y2,z2, ...). Shpae: 3*num_agents x 1
        # Undo Permutation of velcontrol based on E
        E3 = np.kron(E,np.eye(3))
        vel_ctrl_stack = np.dot(np.transpose(E3), vel_ctrl_original_stack) # un-permuted array of positions of robots. Shape: 3*num_agents x 1
        vel_ctrl_agents = vel_ctrl_stack.reshape([num_agents, 3])# array with un-permuted control for robots. Shape: num_agents x 3
        ...
        '''
        create full state for each agent
        '''
        all_states = []
        for agent in range(num_agents):
            pose = np.zeros(7) # position and quaternion rotation
            twist = np.zeros(6) # velocity and angular velocity
            acc = np.zeros(3) # acceleration
            pose[0:3] = np.array(safe_goal_pos_list[agent])
            pose[3:8] = np.array([0,0,0,1])
            twist[0:3] = np.array(vel_ctrl_agents[agent])
            s = state(pose, twist, acc)
            all_states.append(s)
        ...
        #     cf.cmdFullState(all_states[cntr].pose, all_states[cntr].twist, all_states[cntr].acc)

        for n in range((itr_cntr % num_radios), num_agents, num_radios):
            allcfs.crazyflies[n].cmdFullState(all_states[n].pose, all_states[n].twist, all_states[n].acc)
    ...
    # land the crazyflies
    try:
        cur_pos_list = get_current_pos_list(allcfs.crazyflies)
        all_states = []
        for agent in range(num_agents):
            pose = np.zeros(7) # position and quaternion rotation
            twist = np.zeros(6) # velocity and angular velocity
            acc = np.zeros(3) # acceleration
            pose[0:3] = np.array(cur_pos_list[agent])
            pose[3:8] = np.array([0,0,0,1])
            s = state(pose, twist, acc)
            all_states.append(s)
        for cntr, cf in enumerate(allcfs.crazyflies):
            # cf.goTo(goal = np.array(all_states[cntr].pose[0:3]), yaw = 0, duration = 2.0, groupMask = 0)
            cf.cmdFullState(all_states[cntr].pose, all_states[cntr].twist, all_states[cntr].acc)
    except:
        pass

    done = fullStateLandAllCfs(swarm, landing_heights=[])
